#!/usr/bin/env python
#coding:utf-8
"""
__title__ = ""
__author__ = "adw"
__mtime__ = "2016/6/19"
__purpose__ = 
"""
import sys
import jieba
import numpy
from sklearn import metrics
from sklearn.feature_extraction.text import HashingVectorizer
from sklearn.naive_bayes import MultinomialNB


def input_data(train_file, test_file):
    train_words = []
    train_tags = []
    test_words = []
    test_tags = []
    with open(train_file, 'r') as f1:
        for line in f1:
            tks = line.strip().split(",")
            train_words.append(tks[0])
            train_tags.append(tks[2])
    with open(test_file, 'r') as f1:
        for line in f1:
            tks = line.strip().split(",")
            test_words.append(tks[0])
            test_tags.append(tks[2])
    return train_words, train_tags, test_words, test_tags


with open('stopword.txt', 'r') as f:
    stopwords = set([w.strip() for w in f])
comma_tokenizer = lambda x: jieba.cut(x, cut_all=True)


def vectorize(train_words, test_words):
    v = HashingVectorizer(tokenizer=comma_tokenizer, stop_words=stopwords, n_features=30000, non_negative=True)
    train_data = v.fit_transform(train_words)
    test_data = v.fit_transform(test_words)
    return train_data, test_data


def evaluate(actual, pred):
    m_precision = metrics.precision_score(actual, pred)
    m_recall = metrics.recall_score(actual, pred)
    print 'precision:{0:.3f}'.format(m_precision)
    print 'recall:{0:0.3f}'.format(m_recall)


def train_clf(train_data, train_tags):
    clf = MultinomialNB(alpha=0.01)
    clf.fit(train_data, numpy.asarray(train_tags))
    return clf


def main():
    # if len(sys.argv) < 3:
    #     print '[Usage]: python classifier.py train_file test_file'
    #     sys.exit(0)
    train_file = r"F:\17MedPro\workspace\medstdy\docs\data.csv" #sys.argv[1]
    test_file = r"F:\17MedPro\workspace\medstdy\docs\data_test.csv"#sys.argv[2]
    train_words, train_tags, test_words, test_tags = input_data(train_file, test_file)
    train_data, test_data = vectorize(train_words, test_words)
    clf = train_clf(train_data, train_tags)
    pred = clf.predict(test_data)
    print pred
    # evaluate(numpy.asarray(test_tags), pred)


if __name__ == '__main__':
    main()